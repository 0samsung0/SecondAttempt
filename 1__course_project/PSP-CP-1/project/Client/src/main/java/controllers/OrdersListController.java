...
public class OrdersListController implements Initializable {
    ...
    void searchOrder(ActionEvent event) {
        if(number.getText() == "")
            ...
            Connect.client.sendMessage("findOrder");
            Order rd = new Order();
            rd.setId_user(Integer.parseInt(number.getText()));
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        numberColumn.setCellValueFactory(field -> new SimpleObjectProperty<>(field.getValue().getId()));
        clientIdColumn.setCellValueFactory(field -> new SimpleObjectProperty<>(field.getValue().getId_user()));
