            ...
            while (true) {
                ...
                switch (choice) {
                    ...
                    case "registrationStudent" -> {
                        ...
                        SQLFactory ssqlFactory = new SQLFactory();
                        Client client = (Client) sois.readObject();
                        System.out.println(client.toString());

                        Role r = ssqlFactory.getClients().insert(client);
                        System.out.println((r.toString()));
                    ...
                    case "getChartReceive" -> {
                        ...
                        for (Receive r : receives) {
                          data.add(new AbstractMap.SimpleEntry<String, Double>( r.getDate(), 1.0));
                        }

                        //ArrayList<String> data = new ArrayList<>();
                        //for (Receive r : receives) {
                            //data.add(new String( r.getDate()));
                            //}
